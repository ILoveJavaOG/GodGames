...
public class Command_TnTRun extends AbstartcCommands{
	...
	public boolean command(CommandSender sender, String[] args) {
		if(sender instanceof Player) {
			...
			if(!p.hasPermission("MGS.Admin")) {p.sendMessage(Utils.getPrefix() + "§cAchtung, du darfst das nicht!"); return true;}
			if(args.length == 0) {
				...
				if(args[0].equalsIgnoreCase("create")) {
					...
					}else {p.sendMessage(Utils.getPrefix() + "§cAchtung, der Name §e" + name + "§c ist bereit erstellt!"); tnt = null;}
					...
					}else {p.sendMessage(Utils.getPrefix() + "§cAchtung, die Tnt Arena wurde noch nicht erstellt! -> /tntrun create [Name]");}
					...
					}else {p.sendMessage(Utils.getPrefix() + "§cAktion nicht möglich!");}
				}
