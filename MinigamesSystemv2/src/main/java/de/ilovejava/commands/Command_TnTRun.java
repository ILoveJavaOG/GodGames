			...
			if (args.length == 0) {
				...
				if (args[0].equalsIgnoreCase("create")) {
					...
					if (Utils.getTntcreate().containsKey(name)) {
						...
						p.sendMessage(Utils.getPrefix() + "§aDer Lobby Spawn f§r die Arena §e" + name + "§a wurde gesetzt! -> /tntrun setPlay [Name]");
					} else {
					...
					if (Utils.getTntcreate().containsKey(name)) {
						...
						p.sendMessage(Utils.getPrefix() + "§aDu hast den Spiel Spawn f§r die Arena §e" + name + "§a gesetzt! -> /tntrun setSpec [Name]");
					} else {
						p.sendMessage(Utils.getPrefix() + "§cAchtung, die Tnt Arena wurde noch nicht erstellt! -> /tntrun create [Name]");
					}
					...
					if (Utils.getTntcreate().containsKey(name)) {
						...
						p.sendMessage(Utils.getPrefix() + "§aDu hast den Spec Spawn f§r die Arena §e" + name + "§a gesetzt! -> /tntrun setName [Name] [MapName]");
					} else {
						p.sendMessage(Utils.getPrefix() + "§cAchtung, die Tnt Arena wurde noch nicht erstellt! -> /tntrun create [Name]");
					}
					...
					if (Utils.getTNTReady().containsKey(name)) {
						...
						p.sendMessage(Utils.getPrefix() + "§cAktion nicht m§glich!");
					}
				}
